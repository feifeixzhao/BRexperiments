...
dt = mean(diff(time));
nt=numel(time);
Z(1,:)=-1.445; % insert extra row first
Z_local=(Z-Z(1,1)).*1000;
% plot section
nt=numel(time);
smpInterval = 1;
figure(1)
...
%%
figure;
bar(l, setPB, 'FaceColor', [0.8, 0.8, 0.8], 'EdgeColor', 'none'); % Bar graph for setrnd1
hold on;
bar(l, set_obs, 'FaceColor', [0.5, 0.5, 0.9], 'EdgeColor', 'none'); % Bar graph for setPB
hold off;

[h,p,stats] = chi2gof(l, 'Ctrs', l, 'Frequency', setpd_obs, 'Expected', setpd)

% fitting observational data from experiments
pd_pred=fitdist(setrnd1(:), 'Exponential');
setpd_obs=pdf(pd_pred, l); 

% plot fit on histogram
figure
[heights, locations]=histcounts(setrnd1,30);
heights=heights/(length(setrnd)*width); 
h=bar(l.*1000, heights, 'histc'); 
set(h, 'FaceColor', 'w', 'EdgeColor', 'black');
% hold on
plot([0 l], [0 setpd], 'r')
hold on 
plot([0 l].*1000, [0 setpd_pred], 'black')

legend('Predicted', 'Observed')
title('Run 4, Q = 0.12 L/s, S = 0.02')
xlabel(t, 'Set thickness (mm)')
